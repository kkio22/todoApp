...
public interface ScheduleRepository extends JpaRepository <Schedule, Long> { //JpaRepository를 상속받은 레파지토리를 simpleRepository 인터페이스
    ...
    @Query("select s from Schedule s left join fetch s.comments c where s.id = :scheduleId order by s.createdAt asc")
    Optional<Schedule> findByIdWithComment(@Param("scheduleId") Long scheduleId);//Param에 들어갈 건 where로 나타냄, 이건 그냥 comment 같이 가져오게만 하는 쿼리문이야
